...
std::map<int, double> DetectionEvaluator::per_class_ap() const {
    ...
    for (int class_id : class_ids_) {
        ...
        result[class_id] = average(finite_values);
    }
...
void PipelineEvaluator::update(const cv::Mat& gt_mask, const cv::Mat& pred_mask, const std::vector<Box>& gt_boxes, const std::vector<Box>& pred_boxes) {
    segmentation_evaluator_.update(gt_mask, pred_mask);
    classification_evaluator_.update(gt_boxes, pred_boxes);
    detection_evaluator_.update(gt_boxes, pred_boxes);
}

EvaluationReport PipelineEvaluator::report() const {
    return {segmentation_evaluator_.report(), classification_evaluator_.report(), detection_evaluator_.report()};
}
