...
#else
    const std::filesystem::path python_executable = project_root / ".venv" / "bin" / "python";
#endif
...
#else
    const std::string command = "\"" + python_executable.string() + "\" \"" + inference_script.string() + "\" --image_dir \"" + image_dir.string() + "\" --output_dir \"" + output_dir.string() + "\"";
#endif
